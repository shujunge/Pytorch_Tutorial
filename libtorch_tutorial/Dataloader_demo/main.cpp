...
struct Options {
    ...
    std::string datasetPath = "/my_400G/my_dataset/101_ObjectCategories/";
...
void train(T& network, DataLoader& loader, torch::optim::Optimizer& optimizer, size_t epoch, size_t data_size) {
    size_t index = 0;
    network->train();
    float Loss = 0, Acc = 0;
...
template <class T, typename DataLoader>
void test(T& network, DataLoader& loader, size_t data_size) {
    size_t index = 0;
    network->eval();
...
int main() {
    ...
    auto test_set =
            CustomDataset(data.second)
                    .map(torch::data::transforms::Normalize<>({0.5,0.5,0.5}, {0.5,0.5,0.5}))
                    .map(torch::data::transforms::Stack<>());
    int test_size = test_set.size().value();
    auto test_loader =
            torch::data::make_data_loader<torch::data::samplers::SequentialSampler>(
    ...
    at::Tensor images = torch::ones({2,3,224,224});
    auto out  = network->forward(images);
